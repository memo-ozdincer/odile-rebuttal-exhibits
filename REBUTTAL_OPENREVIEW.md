...
- **InjecAgent** (different benchmark, ReAct format, attack domains absent from training): ASR falls to 0.0 on every backbone (worst case 0.15), n=1054/family, Wilson upper bound 0.35% at Qwen-2.5-14B. One subtlety matters for the comparison: InjecAgent's parser only counts outputs in its strict `Thought:/Action:` ReAct format, so any defense that emits a different format scores a low ASR for free; we therefore report ASR alongside valid-rate (the fraction of outputs the parser can read). ReasAlign reaches ≈0 ASR, but as a parser artifact rather than a defense: it was fine-tuned to wrap responses in its own reasoning tags, so even with no attack present the ReAct parser rejects ~94% of its outputs (valid-rate 6.2% at baseline, 0.3% under attack), counting neither attacker nor benign tool calls. ODILE keeps the model in its native ReAct format (valid-rate 43.7%) while driving ASR to 0, a genuine defense rather than defense-by-output-mangling. The attacks share no surface form with training: we invite the reviewer to compare the [124 real InjecAgent attacks]({ANON}/examples_pdf/injecagent_attacks.pdf) against [the AgentDojo injections ODILE trained on]({ANON}/examples_pdf/agentdojo_train_injections.pdf). Full numbers: [InjecAgent table]({ANON}/T3b_injecagent_crossmodel.pdf).
...
- **TAP and PAIR** (semantic attackers given ODILE's objective): 0/64 across two backbones, comprising 0/48 at the standard tree-search budget plus 0/16 when every surviving cell is re-attacked with a deeper tree (depth 5, branching 3, width 6, 2 parallel streams, against the standard depth 3, branching 2, width 4, 1 stream), against base 36/48 and a sanitizer firewall 15/48 on the identical cells. Base and sanitizer cracking those exact cells is the positive control. [TAP/PAIR table]({ANON}/tab_adaptive_tap_pair.pdf). This covers the objective-aware axis.
