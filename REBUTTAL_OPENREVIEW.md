...
**Minor (Figure 2 legend).** We will declutter and split the Llama/Qwen panels. In addition, **R3/R4:** BFCL and Tau-bench retain capability ([table]({ANON}/tab_capability.pdf)) across seven backbones including Qwen3-Next-80B.
...
Six backbones plus Qwen3-Next-80B, including Qwen3-8B and Qwen3-32B from the Qwen-3 family the reviewer points to; Qwen3-Next-80B is additionally a frontier-agentic open-source model (nonstandard Gated-DeltaNet attention), and ODILE transfers across all of them ([cross-model]({ANON}/T2_crossmodel_AD.pdf)).

**3. "Single-benchmark focus [...] additional benchmarks such as AgentDyn and WASP would strengthen the claims."**
Both added, and we report benign utility on each as well as ASR. **AgentDyn** (GitHub, Daily-Life, Shopping; absent from AgentDojo): ASR base 22.1% to ODILE 0.0% (Qwen3-8B, n=560, Fisher p < 1e-12), with benign utility **[AgentDyn benign: base XX% to ODILE YY%, Memo to fill]** ([table]({ANON}/tab_agentdyn.pdf)). **WASP**: attacker-action from 12.5% to 0/239 verified-tight (0/5805 per paraphrase), while benign utility is preserved (base/ODILE benign %: Llama-3.3-70B 100/93, Qwen-2.5-7B 100/95, Qwen-2.5-14B 69/69, Llama-3.1-8B 100/83) ([table]({ANON}/tab_T_wasp.pdf)). The full containerized WASP is a camera-ready commitment.
