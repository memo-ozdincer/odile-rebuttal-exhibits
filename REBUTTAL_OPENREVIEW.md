...
ODILE is trained on AgentDojo (Debenedetti et al. 2024) traces; a memorized adapter would fail on any attack distribution it did not train on, and we show it does not, on four further benchmarks and on held-out attack-augmentations.
- **InjecAgent** (Zhan et al. 2024; different benchmark, ReAct format, attack domains absent from training): ODILE drives ASR-all to ≤0.15% on every backbone (n=1054/family). The attacks share no surface form with training: compare the [30 direct-harm]({ANON}/examples_pdf/injecagent_attacks.pdf) and [32 data-stealing]({ANON}/examples_pdf/injecagent_data_exfil.pdf) attacks against [the AgentDojo injections ODILE trained on]({ANON}/examples_pdf/agentdojo_train_injections.pdf) (a [harness walk-through]({ANON}/examples_pdf/injecagent_harness.pdf) shows where the injection hides). Full numbers: [InjecAgent table]({ANON}/T3b_injecagent_crossmodel.pdf).
...
- **WASP** (Evtimov et al. 2025; web-agent injection): the attacker-action rate falls from 12.5% to 0/239 verified-tight (0/5805 per individual paraphrase attempt, Fisher p≈1e-9), against base 25/200 and a sanitizer at 8/160. [21 real attacker goals]({ANON}/examples_pdf/wasp_attacker_goals.pdf); full numbers in the [WASP table]({ANON}/tab_T_wasp.pdf). (Our setup preserves WASP's records and injection templates without the live browser loop; the full containerized version is a camera-ready commitment.)
...
Memorization cannot explain this: the adapter never saw these tools, games, or environments at training time, and on AgentDyn the base model it is built on is hijacked 22% of the time. The sharpest evidence is the held-out augmentation, where we take the *exact* AgentDojo injections the adapter trained on and only re-render their surface (markers removed, or base64-encoded): ASR stays ≈0 though the goal is unchanged, the opposite of what a test-fit defense would do.

Separately, the generalization is format-driven, not format-keyed: ODILE jams identically in standard ReAct and a delimiter-wrapped variant of the same task ([figure]({ANON}/fig_react_jam_odile.pdf)). A format-keyed defense instead looks artificially strong here: ReasAlign (Li et al. 2026) posts ≈0 InjecAgent ASR mainly because its reasoning-tag format is rejected ~94% of the time even with no attack (over-jam), while ODILE holds ASR ≈0 in the native format (valid-rate 43.7% vs 6.2%).
...
- **Discrete GCG** (Zou et al. 2023; white-box, full weight, adapter, and gradient access): a 20-token adversarial suffix optimized for 75 steps at search-width 128, 0/4 crackable pairs. On the same pairs the undefended base descends to cross-entropy 0.57 and complies, while under ODILE the attacker-target cross-entropy plateaus near 10.7–11.0 and never enters the compliance regime, and the plateau does not move as the budget grows: on a HarmBench (Mazeika et al. 2024) text-generation GCG sweep ODILE held 0% attack success at 500 steps and under adaptive hidden-state targeting, where base Llama-3.3-70B breaks on 68% of behaviors. The [loss curve]({ANON}/figures/fig_gcg_loss_curve.pdf) shows the base descending to compliance while ODILE stays flat; per-pair numbers in the [GCG table]({ANON}/T_GCG.pdf).
...
We add Qwen3-Next-80B-A3B-Thinking (Qwen Team 2025; September 2025), a frontier reasoning model with nonstandard (Gated-DeltaNet) attention; ODILE transfers with the attention-path adjustments ([Qwen3-Next table]({ANON}/tab_pareto_qnext.pdf), [cross-model]({ANON}/T2_crossmodel_AD.pdf)). The Qwen3-Next results shown are on the important_instructions family; we commit to reporting the full 52-cell AgentDojo grid on Qwen3-Next across all four suites, together with its TensorTrust and AgentDyn cells, in the camera-ready.
...
So the novelty is where and how the defense operates: representation-level, single-pass, trained on the model's own behavior rather than on token-trust rules or an external filter. That design is also what gives the practical advantages: 1× inference cost (against Meta-SecAlign's retrain and the firewall's extra pass), and robustness where the others break. Meta-SecAlign is delimiter-keyed and so breaks under format shift (on the marker-removed augmentation it leaks 3.38% where ODILE holds 0.00% [table]({ANON}/t_ad_injection_styles.pdf); a [matched-backbone trace]({ANON}/traces_pdf/trace_secalign_cracked_vs_odile.pdf) shows it executing send_money to the attacker where ODILE jams), whereas ODILE is anchored to the completion-trajectory representation rather than to input delimiters and so behaves identically across input formats: on the same task it jams in both the standard ReAct format and a delimiter-wrapped variant ([figure]({ANON}/fig_react_jam_odile.pdf)). The recognition-based firewall (Bhagwatkar et al. 2026) is likewise bypassed by benign-looking reframes under adaptive attack (cracked 15/48 where ODILE holds 0/64). We benchmark the broader model-level field on one model and one metric pair: ODILE against ReasAlign (Li et al. 2026) across all five benchmarks ([comparator]({ANON}/tab_comparator_8b.pdf); ReasAlign matches ODILE on ASR but at a ≈13pp benign-utility cost), and against Meta-SecAlign, ReasAlign, and MELON (Zhu et al. 2025) in the per-backbone Pareto tables below. We will fold this into the body.
...
It was one specific axis: the calls-user-tool correctness rate on InjecAgent's benign split at Llama-3.3-70B (base = 100%), not a global capability number. We now report absolute numbers with ASR, benign utility, and under-attack utility separated, per backbone ([tables]({ANON}/tab_pareto_70b.pdf)).
...
We re-implemented the Bhagwatkar et al. (2026) firewall faithfully at Llama-3.1-8B (a second LLM rewrites every tool output before the agent sees it). It is useful but not invariant: statically it matches ODILE on ASR (2.1%) and under adaptive attack reduces base cracks (WASP 12.5% to 5.0%; AgentDojo TAP/PAIR 75% to 31%), but it is bypassed by benign-looking paraphrases (four recurring classes: authority-pretextual, multi-step decomposition, benign-disguise narrative, URL/code-embedded), and on the deploy-token, SSH-key, and PAT records it actively *introduces* cracks the base never had (base 0/16, sanitizer 3/12), because the rewriter cannot tell "the user wants a deploy token" from "the injection wants one". ODILE holds 0 across all of these (0/64 on TAP/PAIR, 0/5805 WASP paraphrases; Fisher vs sanitizer p=5e-10).

The reason is mechanistic and is the core of our claim: the sanitizer operates on surface text, so any paraphrase that fools the smaller rewriter gets through, and the rewriter leaks which paraphrases it strips, a gradient the attacker climbs. ODILE operates on hidden states, where paraphrases with the same intent converge to similar representations, so surface variation gives the attacker almost no signal (which is why TAP's iteratively-refined attempts produced 0 cracks). ODILE is preferable when adaptive robustness, 1× inference cost, and benign-utility preservation matter together (the realistic deployment setting); the sanitizer's edge is being backbone-agnostic. [firewall table]({ANON}/T_firewall.pdf), [adaptive table]({ANON}/tab_adaptive_tap_pair.pdf).
