...
In addition, **R5 (defense comparison):** matched-backbone [traces]({ANON}/traces_pdf/trace_secalign_cracked_vs_odile.pdf) and a [firewall comparison]({ANON}/T_firewall.pdf) show where Meta-SecAlign and a sanitizer break while ODILE holds.
...
**vs Meta-SecAlign** (Chen et al. 2026): we hold the low-ASR corner (0.01% vs 2.11%) at 1x cost ([70B table]({ANON}/tab_pareto_70b.pdf)). The differentiator is format robustness: Meta-SecAlign is marker-keyed and breaks under format shift, leaking 3.38% on the marker-removed augmentation where ODILE holds 0.00% ([table]({ANON}/t_ad_injection_styles.pdf)), and ODILE jams identically across ReAct and delimiter formats ([figure]({ANON}/fig_react_jam_odile.pdf); InjecAgent ReAct ASR ≤ 0.15% on every backbone).
...
In addition, **R3 (capability):** BFCL and Tau-bench confirm general tool-use is retained ([table]({ANON}/tab_capability.pdf)).
