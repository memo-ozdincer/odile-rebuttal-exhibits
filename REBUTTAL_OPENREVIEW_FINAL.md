...
The jam is the design intent, not brittleness: under attack ODILE emits no parseable tool call, so the attacker action is never produced. On the same task where the base model calls `update_password`, ODILE reads the file and emits token-soup with no tool call ([trace](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/traces_pdf/trace_base_executes_vs_odile_jams.pdf)). For the three mechanisms the reviewer names:
