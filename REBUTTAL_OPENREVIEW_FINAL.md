...
The three defenses act at different layers. Meta-SecAlign changes the weights (DPO to obey a privileged-instruction hierarchy, keyed to data-delimiters); a firewall/sanitizer rewrites the inputs in a second pass; ODILE reshapes the hidden states on the intent-formation window so a harmful-intent trajectory collapses to a non-actionable (jam) state, in the same single forward pass. The novelty is the representation-level, single-pass, own-behavior-trained mechanism, which also gives 1x inference cost and robustness where the others break: Meta-SecAlign is delimiter-keyed and leaks 3.38% on the marker-removed augmentation where ODILE holds 0.00% ([trace](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/traces_pdf/trace_secalign_cracked_vs_odile.pdf)), and ODILE is format-agnostic across ReAct vs delimiter-wrapped input ([figure](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/fig_react_jam_odile.pdf)). We benchmark the broader model-level field on one model and one metric pair: ODILE vs ReasAlign (Li et al. 2026) across all five benchmarks ([comparator](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/tab_comparator_8b.pdf)); ReasAlign matches ODILE on ASR but at a roughly 13pp benign-utility cost, and its near-0 InjecAgent ASR is a format-rejection artifact (its reasoning-tag format is rejected about 94% of the time), not a defense.
