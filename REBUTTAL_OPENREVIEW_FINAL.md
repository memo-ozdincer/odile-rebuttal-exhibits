...
**vs Meta-SecAlign** (Chen et al. 2026): we hold the low-ASR corner (0.01% vs 2.11%) at 1x cost ([70B table](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/tab_pareto_70b.pdf)). The differentiator is the ReAct agentic format: on InjecAgent, Meta-SecAlign cannot emit valid ReAct tool calls (88–95% of its outputs are unparseable across families, so its low ASR reflects broken output, not defense), whereas ODILE drives ASR to ≤ 0.15% while producing valid ReAct tool calls on every backbone ([figure](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/fig_react_jam_odile.pdf)).
