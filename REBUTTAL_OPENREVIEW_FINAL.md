...
- **InjecAgent** (Zhan et al. 2024; different benchmark, ReAct format, attack domains absent from training): ODILE drives ASR-all to ≤0.15% on every backbone (n=1054/family, Wilson upper bound 0.35% at Qwen-2.5-14B). The attacks share no surface form with training: compare the [30 direct-harm](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/examples_pdf/injecagent_attacks.pdf) and [32 data-stealing](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/examples_pdf/injecagent_data_exfil.pdf) attacks against [the AgentDojo injections ODILE trained on](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/examples_pdf/agentdojo_train_injections.pdf); a [harness walk-through](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/examples_pdf/injecagent_harness.pdf) shows where the injection hides and the base-vs-ODILE branches. Full numbers: [InjecAgent table](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/T3b_injecagent_crossmodel.pdf).
...
- **(a) How we handle them.** The three banking pairs are structurally ambiguous: the attacker IBAN equals the legitimate recipient, so at the middle layers ODILE's contrastive objective operates on, the harmful and benign twins converge to nearly the same representation, giving it almost no signal to separate them. We exclude them by a hard-coded structural rule (target-collision in the task definition), not a per-sweep statistic.
...
We are grateful for the positive assessment and would be glad to add any further detail. We would be grateful for the reviewer maintaining or raising the score.
...
ODILE's novelty is being a representation-level LoRA at 1x inference cost (no second model pass, no input rewriting), trained by a paired-trace contrast on the model's own harmful direction. Against the defenses named:

- **vs Meta-SecAlign** (Chen et al. 2026), head-to-head at Llama-3.3-70B. On **AgentDojo**, ODILE holds 0.01% ASR vs Meta-SecAlign's 2.11% (≈200x lower) at 1x cost, while Meta-SecAlign retrains the full model and carries higher benign utility (62.6% vs 51.5%); they sit at different points on the security-utility frontier ([70B table](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/tab_pareto_70b.pdf)). On **InjecAgent**, ODILE holds ASR-all ≤ 0.15% on every backbone vs Meta-SecAlign's 0.2–2.6% ([InjecAgent table](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/t_injecagent_full.pdf)).
- **Format robustness:** ODILE is also robust to format shift where Meta-SecAlign is not. On InjecAgent's ReAct format Meta-SecAlign cannot emit valid tool calls (88–95% of its outputs are unparseable, so its low ASR is broken output, not defense), whereas ODILE stays in valid ReAct format and jams identically across ReAct and delimiter-wrapped formats ([figure](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/fig_react_jam_odile.pdf)).
- **vs firewall/sanitizer** (Bhagwatkar et al. 2026): a second LLM rewrites each tool output. Static parity (2.1% ASR), but it costs −6.2pp benign utility plus an extra LLM call per output, and is cracked 15/48 under adaptive attack where ODILE holds 0/64 ([firewall table](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/T_firewall.pdf)).
- **Broader field:** the [cross-benchmark comparator](https://anonymous.4open.science/r/odile-anon-exhibits-82D6/tab_comparator_8b.pdf) places ODILE head-to-head with ReasAlign (Li et al. 2026) on all five benchmarks at once; ReasAlign matches ODILE on ASR but at ≈13pp AgentDojo benign cost and ≈4pp BFCL (and on InjecAgent its near-zero ASR is the same format-rejection artifact, valid-rate 0.3% vs ODILE's 43.7%). We also benchmark adjacent stack layers so the comparison is not single-family: MELON (Zhu et al. 2025), a runtime detector, and CaMeL (Debenedetti et al. 2025), a design-level planner/interpreter (per-backbone Pareto tables in answer 2).
...
CaMeL (Debenedetti et al. 2025) is a design-level defense (a privileged planner plus a capability-checked restricted-Python interpreter), a different layer of the stack than ODILE's representation-level intervention, so the two are complementary. On open weights it mostly fails to run, which is the substance of the comparison. On Qwen-2.5-14B the privileged LLM cannot drive the interpreter: it refuses, hallucinates tool outputs, or loops on the interpreter's own error feedback, so benign utility floors at 0% (undefended base 46%) and the 0% ASR is vacuous (no task completes in either direction). On Llama-3.3-70B only two of the four AgentDojo suites complete (workspace 0.90 benign, slack 0.50, both at 0% ASR), while banking and travel crash on a CaMeL interpreter bug (a `ValueError` on a malformed format code the model emits), which is an implementation failure, not a defense result. To give CaMeL its best case we therefore ran it on a frontier model, Gemini-2.5-flash-lite: there all four suites complete at ~0% ASR, but benign utility is only 18–32% per suite (banking 28–31%, slack 17–29%, workspace 19–22%, travel 0%). So CaMeL reaches ~0% ASR by design wherever its interpreter runs, but it is brittle on open weights and high-tax even at frontier scale; ODILE reaches 0% ASR at 1x cost on every backbone with benign utility preserved. For camera-ready we will extend the CaMeL comparison to additional frontier-grade backbones where its interpreter runs, so it is not limited to the single Gemini configuration.
