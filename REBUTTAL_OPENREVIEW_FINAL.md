...
ODILE trains on AgentDojo (Debenedetti et al. 2024), so to test for leakage we ran it on four further benchmarks and a held-out attack-augmentation set, none seen at training; a memorized adapter would fail on these, and it does not, each reported with sample size, significance, a comparator, and a concrete demonstration that it is genuinely different.
