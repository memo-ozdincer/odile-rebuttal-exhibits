...
- **InjecAgent** (Zhan et al. 2024; different benchmark, ReAct format, attack domains absent from training): ODILE drives ASR-all to ≤0.15% on every backbone (n=1054/family, Wilson upper bound 0.35% at Qwen-2.5-14B), at or below Meta-SecAlign (Chen et al. 2026), the closest model-level defense (0.2–2.6% ASR-all across the four families), the most directly comparable benchmark for a model-level defense. We report ASR with valid-rate, because InjecAgent's strict parser scores any off-format output as a non-attack: ReasAlign (Li et al. 2026) reaches ≈0 ASR largely because its reasoning-tag format is rejected ~94% of the time (valid-rate 6.2%), whereas ODILE reaches 0 ASR while staying in the native ReAct format (valid-rate 43.7% at 8B). The attacks share no surface form with training: compare the [30 direct-harm]({ANON}/examples_pdf/injecagent_attacks.pdf) and [32 data-stealing]({ANON}/examples_pdf/injecagent_data_exfil.pdf) attacks against [the AgentDojo injections ODILE trained on]({ANON}/examples_pdf/agentdojo_train_injections.pdf); a [harness walk-through]({ANON}/examples_pdf/injecagent_harness.pdf) shows where the injection hides and the base-vs-ODILE branches. Full numbers: [InjecAgent table]({ANON}/T3b_injecagent_crossmodel.pdf).
